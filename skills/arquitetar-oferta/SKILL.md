...
1. **Check CLAUDE.md first.** Claude auto-loads `~/.claude/CLAUDE.md` and any project-level `CLAUDE.md`. Scan for:
   - **Business** — what the user sells, current pricing tiers, delivery model.
   - **ICP** — pain points, objections, the words they use. Promise/Path/Why Now must speak to THIS person.
   - **Voice** — tone, do/don't, sample sentences. Offer copy must sound like the user wrote it.
2. **If missing, check the project folder** for `Context/business.md`, `Context/icp.md`, `Context/voice.md`. Read whichever exist.
3. **If voice is missing after both:** flag it — "I'll write the offer copy generic-tone and you can pass it through /anti-slop after."

### Step B: Confirm the brief (≤2 questions, combined)

Skip anything context already covers. Combine into at most 2 questions:

1. **Framework + offer** — "Which framework are we starting with (Offer Shell, Offer Map, Protagonist, or 90 Days?) and what's the offer (or which existing one are we refining)?"
2. **Why Now + pricing** — "What's the deadline or Why Now angle, and the price point / tier structure?" (Skip the pricing half if business context covers it.)
...
[Context + Brief]
      |
      v
Step 1: STRUCTURE  — Offer Shell (tiers of access)
      |
      v
Step 2: CRAFT      — Offer Map (Promise/Path/Pricing/Why Now)
      |
      v
Step 3: POSITION   — Protagonist (Goal/Steps/Rules)
      |
      v
Step 4: SELL       — 90 Days of Offers (Red Velvet + daily rhythm)
      |
      v
Step 5: ANTI-SLOP  — mandatory pass
      |
      v
[Final offer package]
```
...
## structure: build the offer shell
...
For the full framework (Front End cycling, Back End upsells, credit-forward language, life-based offers) see [references/offer-shell.md](references/offer-shell.md).

## craft: write the offer map

**Instruction:** Write the offer copy in 4 parts — Promise, Path, Pricing, Why Now. Good copy isn't written, it's assembled.

**Framework:**

1. **PROMISE** — One promise only. Must pass The Meaning Test (show two people, do they read it the same way?). Mundane > elevated. Outcome OR number.
   - Fails: "Transform your physique" / Passes: "See your six pack"
   - Fails: "Become a productivity master" / Passes: "Finish your workday by noon"
   - If intangible, use the "so you can" bridge: "Install the 3 AI Workflows so you can make more than last quarter."

2. **PATH** — Show HOW. 3 stages, each with Benefits (what they GET, not what they DO) + Checkpoint (straight line to Promise) + Timeframe.
   - Bad checkpoints (disconnected): Belief work -> Strategy -> Implementation
   - Good checkpoints (straight line to "Lose 30 lbs"): Lose first 10 -> next 10 -> final 10

3. **PRICING** — Weekly beats monthly. $100-200/week if early, $250-400/week if you can command it. Half your 1:1 rate if going group, double your group rate if going 1:1.

4. **WHY NOW** — Without urgency, people don't move. Pick scarcity (capped spots), urgency (deadline, price raise), or both.

**Short example (for a $99 Claude Code workshop):**
- Promise: "Have Claude Code installed and producing your first marketing asset by end of the 90-minute session."
- Path: Setup (15 min, working install) -> Skill files (30 min, plug-and-play config) -> First asset (45 min, one piece of content shipped)
- Pricing: $99 one-time
- Why Now: Doors close Friday, next cohort in 6 weeks, price goes to $149
...
For the full framework (Meaning Test, Coaching/Consulting/Mentorship frames, Mundanity Rule, Promise Hierarchy, Path/Timeframe rules, all Why-Now tactics) see [references/offer-map.md](references/offer-map.md).

## position: become the protagonist
...
1. **Your Goal** — Must genuinely excite YOU. Money ("$100k/month profit"), Impact ("help 1000 people get a six pack"), Capture ("$5M/month for clients, $50k for me"), or Build ("Proyecto Casa"). The bravery to declare it matters more than the goal itself.

2. **Your Steps** — How you'll get there. This naturally promotes the Offer Shell and conditions the audience: they were warned they'd be sold to, so they can't complain (you can't get mad about being punched in a boxing gym).

3. **Your Rules** — What you will and won't do. Batman doesn't kill — that's why we feel things about him. Promises attract low-income people, philosophy attracts high-income people.
...
- Steps: Run the $99 workshop monthly, open a 4-week cohort quarterly, document every win publicly.
- Rules: No webinars-while-eating-dinner. No fluff. If you don't ship something live in the session, you get a refund.
...
For the full framework (Netflix content strategy, Big/Small Updates, Minigames, the 5 Conditioning Tactics, Percentage Hits, public/private consequences) see [references/protagonist.md](references/protagonist.md).

## sell: run 90 days of offers
...
For the full framework (insight buckets, top-performing subject lines, recycling rhythm, where-to-sell rules, the 90-offer rotation plan) see [references/90-days-offers.md](references/90-days-offers.md).
...
## example: $99 Claude Code workshop offer
...
### Step 2 — Offer Map (copy)

**Offer name:** Claude Code for Marketers (Live Workshop)

**Promise (passes Meaning Test):**
"Have Claude Code installed, configured for your marketing work, and producing your first asset by the end of a 90-minute live session — even if you've never touched code."

**Path (3 stages, straight line to Promise):**
1. **Setup (0-15 min)** — Working Claude Code install on your machine. Checkpoint: it runs.
2. **Configure (15-45 min)** — My personal skill files plugged in for content, ICP, voice. Checkpoint: Claude knows your business.
3. **Ship (45-90 min)** — Produce one real marketing asset live. Checkpoint: you have something to post tomorrow.
...
### Step 3 — Protagonist Positioning
...
### Step 4 — 90-Day Sell Plan (Red Velvet rotation)

- **Weeks 1-2:** Promote workshop + "doors close Friday" urgency
- **Weeks 3-4:** Same workshop, add bonus (n8n automation pack)
- **Weeks 5-6:** Promote $297 tier (workshop + 2 calls), 5 spots only
- **Weeks 7-8:** Back to $99 workshop, "price goes to $149 next month"
- **Weeks 9-10:** Cohort waitlist open
- **Weeks 11-12:** Recycle best workshop email + open next cohort
...
### Final Output Block (copy-pasteable)
...
**Every Day:**
- Make 1 Front End offer (email + social)
- Post Netflix-style content

**Every Week:**
- Make 1 Back End offer to current clients

**Every 2 Weeks Max:**
- Red Velvet your offer (change something)

**Every Month:**
- Open/close an intake
- Update Protagonist progress
...
- `secondbrain:setup` — populates Context/business.md, icp.md, voice.md before this skill runs
- `vibe2-positioning-angles` — finds the angle before you commit to a Promise
- `entrevista-negocio` — fills business / ICP / voice context (into CLAUDE.md or `Context/*.md`) if it's empty

**Feeds out (use these after to ship the offer):**
- `landing-page` — turns the offer package into a full sales page
- `sequencia-email` — builds the 12-email launch sequence around the Why Now hook
- `escrever-newsletter` — writes the daily insight emails for the 90-day rotation
- `post-longo-x` / `reaproveitar-conteudo` — turns the Protagonist Post into social content
- `anti-slop` — MANDATORY pass on every output before delivery

## the test

Before delivering the offer package, verify each:

1. **Context check:** Did I pull business / ICP / voice from CLAUDE.md or `Context/*.md` before writing? (If anything critical was missing, did I flag it to the user?)
2. **Promise meaning test:** Would two different people read the Promise the same way? Is it mundane (outcome or number), not elevated?
3. **Path checkpoints:** Do all 3 stages connect in a straight line to the Promise, or are they disconnected (belief work -> strategy -> implementation)?
4. **Tiers sell access, not stuff:** Does the price difference between tiers reflect access to the user, not just more deliverables?
5. **Why Now is specific:** Is there a real deadline, price raise, or spot cap? "Limited time" alone doesn't count.
6. **Protagonist voice:** Does the copy sound like the user has a goal, steps, and rules — or like a generic coach who's "always there"?
7. **Anti-slop pass:** Did the draft go through the anti-slop skill (or manual scan) and get cleaned of robot phrases, rhetorical questions, em dashes, throat-clearing?

**Failure condition:** If any check fails — especially #1, #2, or #7 — DO NOT deliver. Fix the failure and re-run the test. Shipping a generic-promise, slop-laden offer is worse than asking the user one more clarifying question.

## source documents

These frameworks synthesized from:
- 90 Days Of Offers Swipe File
- Google Doc 314
- The Offer Shell
- Protagonist Workshop
...
2. If it doesn't exist, create it with header: `# Offer Architect Workshop — Learned Rules`
...
### What to log vs skip
**Log:** pricing rules, positioning preferences, structural choices, voice calibrations.
**Skip:** one-off task fixes, preferences already in Context/voice.md.
