...
Run `humanizar-texto` on every piece before delivery. Final gate, non-negotiable.
...
- `afiar-ideia` — sharpened angles seed the multi-channel campaign
- `post-longo-x` — long-form tweets seed the thread/carousel version

**Runs after this skill (polish):**
- `humanizar-texto` — mandatory final pass on every channel piece
- `polir-texto` — optional second polish for long-form YouTube scripts

For extended worked examples (newsletter → multi-channel, podcast → multi-channel, landing page → multi-channel), see [references/worked-examples.md](references/worked-examples.md).
