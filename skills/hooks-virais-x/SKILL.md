...
Before writing, open [references/hook-examples.md](references/hook-examples.md). This is the source of truth — 140+ categorized patterns with real examples, templates, and "use when" guidance.
...
Pull the specific pattern from `references/hook-examples.md`, not the high-level family. Each entry in the library has a template and a real example — adapt the template to the user's angle.
...
Always check the user's voice guide before finalizing. For example:
...
See [references/worked-example.md](references/worked-example.md) for a complete run — context gathering, pattern selection, 5 hook drafts, pressure-test results, and the final recommendation on a realistic topic.
...
- **/afiar-ideia** — if the angle isn't sharp yet, run that first to generate 3-5 sharper angles, then bring the chosen angle here as input.
- **/vibe2-brand-voice** or user's voice guide — if a voice file exists, the hook should match the user's tone.

**This skill feeds into:**
- **/post-longo-x** — the hook opens the long-form post. That skill writes the body using 9 structural patterns.
- **/escrever-linkedin** — for LinkedIn-specific hooks, the patterns carry over but posting format differs slightly.
- **/escrever-artigo** — for long-form X articles, the hook is still the scroll-stopper.
- **/humanizar-texto** — mandatory final pass on any hook before posting.
...
2. **Uses a specific pattern from the library** — not invented, not generic. If the pattern doesn't exist in `references/hook-examples.md`, rewrite.
...
6. **Fits the user's voice** — if a voice guide exists, the hook passes it. Voice rules can include things like: no em-dash rule-of-three moves, no "not X, it's Y".
